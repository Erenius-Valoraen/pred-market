...
try {
  ...
  // Collapse runs of identical prices left by an older sampler, so a market
  // that sat still overnight doesn't open with a flat line and no movement.
  for (const [k, v] of Object.entries(history)) {
    history[k] = v.filter((s, i) => {
      const prev = v[i - 1];
      return !prev || !prev.p || prev.p.length !== s.p.length || prev.p.some((x, j) => x !== s.p[j]);
    });
  }
} catch { /* first run */ }

/** Record one price point, unless it repeats the last one. */
function recordPrice(slug, prices) {
  const row = (history[slug] ??= []);
  const p = prices.map((x) => Math.round(x * 1000) / 1000);
  const last = row[row.length - 1];
  if (last && last.p.length === p.length && last.p.every((x, i) => x === p[i])) return false;
...
async function sampleHistory() {
  ...
  for (const m of markets) {
    ...
    recordPrice(m.slug, m.prices);
  }
  fs.writeFileSync(HISTORY_FILE, JSON.stringify(history));
}
setInterval(() => sampleHistory().catch(() => {}), 20_000).unref();
sampleHistory().catch(() => {});
...
async function route(req, url, body) {
  ...
  if (url.pathname.startsWith('/api/admin/')) {
    ...
    // The seeding tools trade faster than the sampler ticks, and they cannot
    // write data/history.json themselves: this process holds it in memory and
    // would overwrite their points on the next tick. So they post them here.
    if (url.pathname === '/api/admin/price' && req.method === 'POST') {
      const added = recordPrice(body.slug, body.prices);
      if (added) fs.writeFileSync(HISTORY_FILE, JSON.stringify(history));
